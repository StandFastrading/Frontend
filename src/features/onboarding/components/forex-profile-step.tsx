...
const TRADER_TYPES: Option[] = [
  {
    id: "scalper",
    label: "Scalper",
    description: "Quick in/out for small consistent gains.",
    icon: Gauge,
  },
  {
    id: "day-trader",
  ...
  },
  {
    ...
    description: "Long-term macro and fundamental plays.",
    icon: Compass,
  },
  {
    id: "momentum-trader",
...
const SESSIONS: Option[] = [
  { id: "asian", label: "Asian Session", description: "Tokyo, Sydney, low vol.", icon: Moon },
  { id: "london-open", label: "London Open", description: "Liquidity ignition.", icon: Sunrise },
  { id: "ny-open", label: "New York Open", description: "US session begins.", icon: Sun },
  { id: "overlap", label: "London / NY Overlap", description: "Peak liquidity window.", icon: Globe },
  { id: "after-hours", label: "After Hours", description: "Off-hours and rollovers.", icon: Sunset },
];
...
export function ForexProfileStep() {
  ...
  function handleContinue() {
    console.log("[onboarding] forex.profile", {
      traderType,
      pairs: Array.from(pairs),
      session,
      timeframe,
    });
    router.push("/onboarding/forex/setups");
  }
    ...
    <div className="flex flex-1 flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <h1 className="font-heading text-3xl font-bold tracking-tight text-white sm:text-4xl">
          Your forex trading profile
        </h1>
        <p className="text-sm leading-relaxed text-slate-300">
          Tell us how you typically trade the forex markets.
        </p>
      </div>

      <NumberedSection
        num={1}
        title="What type of forex trader are you?"
      ...
      <NumberedSection
        num={2}
        title="What pairs do you primarily trade?"
      ...
      <NumberedSection
        num={3}
        title="What sessions do you primarily trade?"
        subtitle="The one you're most active in."
      >
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-5">
          {SESSIONS.map((s) => {
            const Icon = s.icon;
            const selected = session === s.id;
            return (
              <PickCard
                key={s.id}
                variant="single"
