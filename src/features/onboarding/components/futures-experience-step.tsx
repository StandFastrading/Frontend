...
const LEVELS = [
  {
    id: "beginner",
    label: "Beginner",
    description: "Learning contracts, leverage, and market structure.",
    icon: BookOpen,
  },
  {
    id: "developing",
    label: "Developing",
    description: "Comfortable with execution but still refining consistency.",
    icon: Sparkles,
  },
  {
    id: "intermediate",
    label: "Intermediate",
    description: "Experienced with structured futures trading.",
    icon: Target,
  },
  {
    id: "advanced",
    label: "Advanced",
    description: "Actively managing multiple products and advanced execution.",
    icon: Star,
  },
] as const;
...
export function FuturesExperienceStep() {
  ...
  function handleContinue() {
    console.log("[onboarding] futures.experience", { level, tenure });
    router.push("/onboarding/futures/profile");
  }
    ...
    <div className="flex flex-1 flex-col gap-8">
      <div className="flex flex-col gap-2">
        ...
        <h1 className="font-heading text-3xl font-bold tracking-tight text-white sm:text-4xl">
          Your futures trading experience
        </h1>
        <p className="text-sm leading-relaxed text-slate-300">
          Help StandFast understand how you trade futures so we can personalize
          your protections and workflows.
        </p>
      </div>

      <Section
        icon={GraduationCap}
        title="How experienced are you with futures trading?"
        subtitle="Pick the one that best fits you today."
      >
      ...
      <Section
        icon={User}
        title="How long have you traded futures?"
        subtitle="No wrong answer — we use this to calibrate guidance."
      >
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {TENURES.map((t) => {
      ...
      <Callout
        icon={Lightbulb}
        title="Leverage rewards precision."
        text="Futures move fast and size matters. We'll calibrate intervention strength to where you are."
      />
