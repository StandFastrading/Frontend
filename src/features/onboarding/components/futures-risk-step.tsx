...
const SPECIAL_RULES: Rule[] = [
  ...
  {
    id: "no-revenge-after-reds",
    label: "No revenge trades after red trades",
  ...
  {
    id: "no-news-trading",
    label: "No trading during major news",
    description: "Pause around scheduled high-impact events.",
    icon: Newspaper,
  },
  {
    id: "no-add-after-losses",
    label: "No additional contracts after losses",
    description: "Lock contract count after the first loss.",
    icon: TrendingDown,
  },
  {
...
export function FuturesRiskStep() {
  ...
  const [maxSameDirection, setMaxSameDirection] = useState(3);
  ...
  return (
    <div className="flex flex-1 flex-col gap-5">
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        ...
        <MetricCard
          title="Max same-direction trades"
          subtitle="In a row before forcing a reset."
        >
          <NumberStepper
            value={maxSameDirection}
            onChange={setMaxSameDirection}
            min={1}
            max={10}
            step={1}
            format={(v) => `${v} trades`}
        ...
        <MetricCard
          title="Max re-entries per contract"
