...
import {
  FUTURES_SETUP_LIBRARY,
  type LibrarySetupCategory,
} from "@/features/onboarding/futures-setups-data";

export type CustomSetup = { id: string; label: string };

export function FuturesSetupLibrary({
  selected,
  onToggle,
  search,
  onSearch,
  ...
  const filtered = useMemo(() => {
    if (!searching) {
      return FUTURES_SETUP_LIBRARY.map((cat) => ({
        ...cat,
        matches: cat.setups,
      }));
    }
        ...
        <div className="flex flex-col gap-2">
          {filtered.map((cat) => (
            <CategoryAccordion
              ...
              matches={cat.matches}
              totalCount={
                FUTURES_SETUP_LIBRARY.find((c) => c.id === cat.id)?.setups
                  .length ?? 0
              }
    ...
    <div className="flex flex-col gap-4 rounded-xl border border-cyan-400/25 bg-cyan-400/[0.04] p-4">
      ...
      {creating && (
        <div className="flex flex-col gap-2 rounded-lg border border-cyan-400/30 bg-[#08111f]/80 p-3 duration-200 animate-in fade-in slide-in-from-top-1">
          <label
            htmlFor="custom-futures-setup-name"
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Input
              ...
              placeholder="e.g. ES 9:30 ORB Pullback"
