...
import {
  OPTIONS_STRATEGY_LIBRARY,
  type LibraryStrategyCategory,
} from "@/features/onboarding/options-strategies-data";

export type CustomStrategy = { id: string; label: string };

export function OptionsStrategyLibrary({
  selected,
  onToggle,
  search,
  onSearch,
  ...
  onAddCustomStrategy: (label: string) => void;
}) {
  ...
  const filtered = useMemo(() => {
    if (!searching) {
      return OPTIONS_STRATEGY_LIBRARY.map((cat) => ({
        ...cat,
        matches: cat.strategies,
      }));
    }
    return OPTIONS_STRATEGY_LIBRARY.map((cat) => ({
      ...cat,
      matches: cat.strategies.filter((s) =>
  ...
  return (
    <div className="flex flex-col gap-4 border-t border-cyan-400/15 p-4">
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-white/[0.08] bg-[#0c1428]/40 p-6 text-center">
          ...
          <p className="mt-1 text-xs text-slate-400">
            Try a different search, or create a custom strategy below.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {filtered.map((cat) => (
            <CategoryAccordion
              ...
              matches={cat.matches}
              totalCount={
                OPTIONS_STRATEGY_LIBRARY.find((c) => c.id === cat.id)
                  ?.strategies.length ?? 0
              }
      ...
      <HelperPanel
        ...
        selected={selected}
        onSelect={onToggle}
        onAddCustom={onAddCustomStrategy}
      />
    </div>
  );
}
...
function CategoryAccordion({
  ...
  category: LibraryStrategyCategory;
  ...
  const isRisk = category.id === "high-risk-behavior";
    ...
    <div
      className={cn(
        ...
        isRisk
          ...
          : "border-white/[0.08] bg-[#0c1428]/60",
      )}
    >
      <button
        type="button"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-slate-400">
            {searching
              ? `${matches.length} of ${totalCount} strategies`
        ...
        <div className="flex flex-col gap-2 rounded-lg border border-cyan-400/30 bg-[#08111f]/80 p-3 duration-200 animate-in fade-in slide-in-from-top-1">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Input
              ...
              placeholder="e.g. SPY 0DTE Breakout Calls"
        ...
        <div className="flex flex-col gap-2 border-t border-white/[0.06] pt-3">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[11px] text-slate-500">
              {customStrategies.length} created
            </p>
          </div>
