...
} from "@/features/onboarding/setups-data";
...
export function SetupLibrary({
  ...
  const filtered = useMemo(() => {
    if (!searching) {
      return SETUP_LIBRARY.map((cat) => ({ ...cat, matches: cat.setups }));
    }
    return SETUP_LIBRARY.map((cat) => ({
      ...cat,
        ...
        <div className="flex flex-col gap-2">
          {filtered.map((cat) => (
            <CategoryAccordion
              ...
              matches={cat.matches}
              totalCount={
                SETUP_LIBRARY.find((c) => c.id === cat.id)?.setups.length ?? 0
              }
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-xs text-slate-300">
              Create one that fits your strategy and workflow.
            </p>
          </div>
        </div>
        {!creating && (
        ...
        <div className="flex flex-col gap-2 rounded-lg border border-cyan-400/30 bg-[#08111f]/80 p-3 duration-200 animate-in fade-in slide-in-from-top-1">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Input
              ...
              placeholder="e.g. ORB Pullback into VWAP"
